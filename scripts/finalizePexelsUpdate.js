...
places.forEach(p => {
  ...
  if (cached) {
    if (cached.status === 'replaced' && cached.new_image_url) {
      p.cover_image = cached.new_image_url;
      p.images = [cached.new_image_url];
      if (cached.match_type === 'exact') exactCount++;
      else relevantCount++;
      if (cached.pexels_photo_id) uniquePhotoIds.add(cached.pexels_photo_id);
    } else {
      keptCount++;
    }
  } else {
...
// AUTO-GENERATED by scripts/finalizePexelsUpdate.js — DO NOT EDIT MANUALLY
...
console.log('===========================================================');
console.log('FINAL PEXELS IMAGE ENRICHMENT AUDIT SUMMARY');
console.log('===========================================================');
console.log(`- TOTAL PLACES IN DATASET: ${places.length}`);
console.log(`- PEXELS EXACT REPLACEMENTS: ${exactCount}`);
console.log(`- PEXELS RELEVANT REPLACEMENTS: ${relevantCount}`);
console.log(`- EXISTING IMAGES RETAINED: ${keptCount}`);
console.log(`- UNIQUE PEXELS PHOTOS USED: ${uniquePhotoIds.size}`);
console.log(`- AUDIT CSV WRITTEN TO: ${auditCsvPath}`);
console.log(`- NEEDS REVIEW CSV WRITTEN TO: ${reviewCsvPath}`);
